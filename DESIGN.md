...
**The export must stream.** A full-window export can span millions of hit rows (4.5), and administrators will run the full window. Loading those through standard Service Builder finders before serializing would exhaust the heap and take down a production JVM. The background task therefore reads one ordered event and hit join through a raw JDBC cursor (a server side cursor, fetching 1,000 rows per round trip), holds only the hits of the event being assembled, and serializes each event to a JSONL line straight into the `ZipOutputStream` as soon as it is complete, so memory stays flat regardless of dataset size. `ActionableDynamicQuery` was the original implementation and was replaced in TO-82: with a hit lookup per event it cost one round trip per event and hydrated millions of Hibernate entities only to discard them, and a 600,000-event range took 1,520 seconds against 136 for the cursor. Streaming depends on the JDBC driver honouring the fetch size: PostgreSQL's and MariaDB's drivers do as shipped, and MySQL Connector/J does only with `useCursorFetch=true` on the connection URL, which the README therefore lists as a requirement on MySQL (TO-105). The archive is assembled on disk, never in memory. This is a hard implementation requirement, not an optimization.
...
| **EC-7** | `SchedulerEngineHelper` vs `BackgroundTaskManager` for purge and export. | Section 3.4. Verify both are public kernel API on 7.4 and pick per job type. | **Resolved.** `SchedulerEngineHelper`, `SchedulerJobConfiguration`, `BackgroundTaskManager` and `BackgroundTaskExecutor` are all public in `release.dxp.api-2025.q1.27`. The split 3.4 proposed is implemented: scheduler for the purge, background task for the export. Availability and compilation verified; the purge firing has not been observed at runtime. |
...
| **EC-14** | Verify Liferay's user notification framework on the target platform (`UserNotificationEventLocalService`, a registered `UserNotificationHandler`) for a plugin-originated notification to a specific user. Confirm the enabling user's ID is available when logging is switched on from the configuration screen. | 3.6 depends on it. Fallback if unavailable: stall and readiness states shown as banners on the plugin's own admin screen only. | **Delivery confirmed; the enabling user's identity is still open.** On a running DXP 2025.Q1.27 instance both notifications are stored, interpreted by the registered handler into the right title, body and link, and shown in the administrator's own notifications list. That last part needed a fix (TO-110): the list and its counter only show events stored as delivered, and the sender first stored them as undelivered, so they reached the table and nobody's feed. The e2e suite now reads that list. The API half: `UserNotificationEventLocalService`, `UserNotificationHandler`, `BaseUserNotificationHandler`, `UserNotificationFeedEntry`, `NotificationEvent` and `UserNotificationDeliveryConstants` are all public in `release.dxp.api-2025.q1.27`, and the sender and handler compile against them. `UserNotificationManagerUtil` resolves a stored event to a handler by matching the event's `type` against the handler's `getPortletId()`, which is why both sides read the portlet name from one constant in the api bundle. Not confirmed: that `PrincipalThreadLocal` carries the enabling administrator inside a `ConfigurationModelListener.onAfterSave` for a company-scoped configuration. That is a reasoned expectation, not an observation: a model listener runs inside the save, on the administrator's own request thread, whereas a `@Modified` callback runs later on a Configuration Admin thread with no principal. It fails safe. An unresolved user id is zero, and notifications then go to the instance administrators, which this section already specifies. A notification that is never delivered leaves the cycle record unchanged, and the admin screen renders the readiness state from that record rather than from delivery, so the fallback is the code that is always running rather than a branch waiting to be written. |
