...
| **EC-2** | Do Search Bar Suggestions requests reach `Searcher`, and can they be reliably classified from within the wrapper? | Determines whether 5.1 exclusion works by direct signal or needs a heuristic. | **Not measured, and the design's premise is now doubtful.** The suggestions endpoint lives in the same `/o/search` application that answers 404 for every resource (see EC-5), so it could not be exercised. Separately, EC-11 shows the wrapper sees the request path as `/c/portal/layout`, the friendly URL having already been resolved before `Searcher` is reached. Section 5.1 classifies suggestion traffic by matching the request URI against `/o/search/v1.0/suggestions`, and that match will not occur on a path that has already been rewritten. Until a real suggestions request is observed reaching the wrapper, suggestion exclusion should be assumed not to work. |
...
| **EC-5** | Does the headless Search API (`/o/search/v1.0/...`, GA in DXP 2025.Q4+) route through `Searcher`? | Coverage completeness for API-driven consumers. | **Not measurable on the test instance; structurally yes.** `SearchResultResourceImpl`'s component descriptor declares `<reference name="_searcher" interface="com.liferay.portal.search.searcher.Searcher"/>`, so the headless path does resolve `Searcher` through the registry and would be wrapped, and it is a plain reference, so it inherits EC-3's static-reluctant constraint. It could not be exercised: every resource in the `/o/search` application answers 404, including `embeddings/embedding-models`, which is not feature-flag gated, while `openapi.json` from the same application answers 200. The feature flag `LPS-179669` was initially blamed and that was wrong: it is enabled on the instance and the endpoints still 404, so the resource components are not registering for some other reason. Diagnosing that needs `scr:list` from a Gogo shell, which is not enabled here, and is a platform question rather than a plugin one. |
