...
| **EC-3** | Is `Searcher` cleanly overridable by `service.ranking`, and what is the correct `@Reference` target filter to avoid self-binding? | Core mechanism. Also: behavior if a second app wraps `Searcher`. Document a distinct ranking value and make delegate lookup defensive. | Open |
