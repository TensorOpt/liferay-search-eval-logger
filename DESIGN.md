...
| **EC-2** | Do Search Bar Suggestions requests reach `Searcher`, and can they be reliably classified from within the wrapper? | Determines whether 5.1 exclusion works by direct signal or needs a heuristic. | **Blocked, and partly answered negatively.** The REST suggestions endpoint is gated behind feature flag `LPS-179669` (off by default), so it could not be exercised. Independently, EC-11 shows the wrapper sees the request path as `/c/portal/layout`, not the friendly URL: the URI-based classification 5.1 relies on is therefore not as dependable as assumed, and needs rechecking against a real suggestions request before 5.1 can be trusted. |
...
| **EC-5** | Does the headless Search API (`/o/search/v1.0/...`, GA in DXP 2025.Q4+) route through `Searcher`? | Coverage completeness for API-driven consumers. | **Blocked by a feature flag, structurally likely yes.** `/o/search/v1.0/search` answers 404 because `SearchResultResourceImpl` opens with `if (!FeatureFlagManagerUtil.isEnabled("LPS-179669")) throw new NotFoundException()`, and `portal.properties` ships that flag false. Setting it through `portal-ext.properties` at both Liferay Home and `WEB-INF/classes` did not take. The resource does hold `@Reference private Searcher`, so the path would be wrapped, and it inherits EC-3's binding constraint. Not measured. |
