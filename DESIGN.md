...
**The export must stream.** A full-window export can span millions of hit rows (4.5), and administrators will run the full window. Loading those through standard Service Builder finders before serializing would exhaust the heap and take down a production JVM. The background task therefore reads one ordered event and hit join through a raw JDBC cursor (a server side cursor, fetching 1,000 rows per round trip), holds only the hits of the event being assembled, and serializes each event to a JSONL line straight into the `ZipOutputStream` as soon as it is complete, so memory stays flat regardless of dataset size. `ActionableDynamicQuery` was the original implementation and was replaced in TO-82: with a hit lookup per event it cost one round trip per event and hydrated millions of Hibernate entities only to discard them, and a 600,000-event range took 1,520 seconds against 136 for the cursor. The archive is assembled on disk, never in memory. This is a hard implementation requirement, not an optimization.
