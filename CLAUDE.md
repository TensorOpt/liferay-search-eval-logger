...
- **Naming collision**: the generated model type `model.ai.tensoropt.sel.SearchHit` collides by simple name with Liferay's `com.liferay.portal.search.hits.SearchHit`. Any code touching both must fully qualify one; the impl and web modules both do.
