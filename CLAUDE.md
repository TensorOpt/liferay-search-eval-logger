...
**Collection implemented; export not.** The Liferay Workspace root (`settings.gradle`, `build.gradle`, `gradle.properties`, Gradle wrapper) and the api, service and impl modules exist. The impl module implements §3 and §5 in full: the `Searcher` wrapper, admission filter, facet/blueprint/cohort capture, Message Bus dispatch and persistence, and the retention purge. The web module does not exist, so there is no admin screen and no export yet. The workspace plugin auto-includes any directory under `modules/` that has a `build.gradle`, so no `settings.gradle` change is needed when it lands.

Two things live in the **api** module rather than where they are used, because more than one bundle needs them: `SearchEvalLoggerConfiguration` (the `@Meta.OCD` interface — the web module will need the same type to read and write per-instance settings, and Liferay's own convention puts configuration interfaces in the api bundle) and `SearchEvalLoggerStatistics` (dispatch/drop/persist counters the export manifest reports). Everything else in the impl module is under `com.tensoropt.search.eval.logger.internal` and exported to nobody.

What round 4 can rely on: `SearchEventLocalService.getActionableDynamicQuery()` for the streaming export (§6.1 — never load the window into memory), `SearchHitPersistence.findBySearchEventUuid` for each event's hits, `SearchEvalLoggerStatistics` for the manifest's drop count (process-wide, resets on restart — say so in the manifest), and `SearchEvalLoggerConfiguration` for the manifest's configuration snapshot. `resource-actions.xml` and the export permission are still unowned and belong to the web module.
